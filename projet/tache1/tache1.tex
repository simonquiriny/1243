...
\chemform{CH_4(g)} et le \chemform{O_2(g)} sont présents en quantité stoechiométrique que : 
...
On sait que 
$$n_{CO_-\text{WGS}} = n_{CO-\text{réformage primaire}}
...
\Delta H_1(T) = \Delta H(\unit{298.15}{\kelvin}) 
 ...
 Nous pouvons donc calculer le $\Delta G_1$
...
Connaissant l'enthalpie en conditions standards \cite{atkins} et les capacités calorifiques dépendant de la température de l'équation précedente et celle-ci \cite{hc-table}:
...
\Delta S_2(T) = \Delta S(\unit{298.15}{\kelvin}) 
 ...
 Nous pouvons donc caluler le $\Delta G_2$
...
% Seb : J'ai rajouté du texte, et ai ajouté quelques modifications. J'ai aussi supprimé quelques 
% 	petites choses inutiles que j'avais faites (calcul du Cp à la température donnée, pas nécessaire).

Afin d'\'evaluer la quantit\'e totale d'\'energie dont nous avons besoin pour mener \`{a} bien la synth\`{e}se d'ammoniac, nous allons regarder les quantit\'es requises \`{a} chaque \'etape du processus pour ensuite d\'eterminer la totalit\'e des besoins \'energ\'etiques du syst\`{e}me.

Comme chaque r\'eaction se passe \`{a} des temp\'eratures diff\'erentes de la temp\'erature ambiante, nous calculerons le $\Delta H_{reaction}$ \`{a} temp\'erature ambiante selon l'\'equation suivante 
$$\Delta H_{reaction} 	= \Sigma \Delta H_{f,produits} - \Sigma \Delta H_{f,reactifs}$$

Ensuite, \`{a} l'aide des $C_{p}$ variables en fonction de la temp\'erature, nous serons alors en mesure de d\'eterminer le $\Delta H_{reaction}$ \`{a} la temp\'erature voulue selon l'\'equation
$$\Delta H(T_{2}) = \Delta H(T_{1}) 
+ \int_{T_{2}}^{T_{1}} C_{p_{reactifs}} dT + \int_{T_{1}}^{T_{2}} C_{p_{produits}} dT$$ 

o\`{u} $T_{1}$ est ici la temp\'erature ambiante, soit \unit{298.15}{\kelvin}.

\paragraph{\'Equation de combustion}
La combustion du m\'ethane se passe dans le four, et fournit la totalit\'e de l'\'energie requise par l'ensemble du processus, avec un rendement de 75 pourcents.
La r\'eaction se produit selon l'\'equation chimique suivante :
\begin{chemmath}
...
$\\$ La r\'eaction se passe g\'en\'eralement \`{a} une temp\'erature avoisinant les $\unit{1300}{\kelvin}$.\\
Voici les $C_{p}$ variables en fonction de la temp\'erature des diff\'erents composants :
$\\C_{p_{CH_{4}}}(\unit{1300}{\kelvin}) :\\ 14.23 + 75.3\cdot10^{-3}\cdotT + (-18\cdot10^{-6})\cdotT^2$
$\\C_{p_{O_{2}}}(\unit{1300}{\kelvin}) :\\ 25.73 + 12.97\cdot10^{-3}\cdotT + (-3.77\cdot10^{-6})\cdotT^2$
$\\C_{p_{CO_{2}}}(\unit{1300}{\kelvin}) :\\ 32.22 + 22.18\cdot10^{-3}\cdotT + (-3.35\cdot10^{-6})\cdotT^2$
$\\C_{p_{H_{2}O}}(\unit{1300}{\kelvin}) :\\ 30.13 + 10.46\cdot10^{-3}\cdotT + (0\cdot10^{-6})\cdotT^2$

$\\$Calculons maintenant le \Delta H pour une temp\'erature $T_{2}$ de $\unit{1300}{\kelvin}$ :

$$\Delta H(\unit{1300}{\kelvin}) = \Delta H(\unit{298.15}{\kelvin}) 
+ \int_{1300}^{298.15} C_{p_{reactifs}} dT + \int_{298.15}^{1300} C_{p_{produits}} dT$$
...
besoin pour notre réacteur multi-tubulaire. Nous avons comme données de base :

$$\dot{m_{NH_{3}}} = \unit{1500}{\ton\per\dday}\text{débit massique par jour d'ammoniac}$$

$$d_{tube} = \unit{10^{-1}}{\meter} \text {diamètre d'un tube}$$ 

$$c = \unit{2}{\meter\per\second}$ où $\dot{m_{NH_{3}}}\text{vitesse superficielle à l'entrée du réacteur}$$
...
avec $\nu$ le volume massique (en \unit{}{\meter\cubed\per\kilo\gram}) du mélange que l'on introduit dans le réacteur. 
\\
